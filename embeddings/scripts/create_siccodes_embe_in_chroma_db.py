...
def create_embeddings():
    ...
    for model_name, model in embedding_models.items():
        ...
        desc_embeddings = model.encode(sic_descriptions, normalize_embeddings=True, show_progress_bar=True)
        section_embeddings = model.encode(section_descriptions, normalize_embeddings=True, show_progress_bar=True)
        ...
        final_embeddings = sic_desc_weight * desc_norm + section_desc_weight * section_norm

        # Save to disk
        np.save(f"./embeddings/file_data/sic_and_section_embeddings_{model_name}.npy", final_embeddings)
        np.save(f"./embeddings/file_data/sic_only_embeddings_{model_name}.npy", desc_norm)
    ...
    # Generate metadata
    metadata = df[["SIC Code", "Description", "Section Name", "Section Description"]].astype(str).to_dict(orient="records")
    ids = [str(uuid.uuid4()) for _ in metadata]

    with open("./embeddings/file_data/sic_metadata.json", "w") as f:
